...
if [ "${APP_ENABLE_CRONTAB:-0}" = "1" ]; then
    (crontab -u www-data -l ; echo "* * * * * /usr/bin/php /var/www/html/artisan schedule:run") | crontab -u www-data -
else
    crontab -u www-data -l | grep -v '/usr/bin/php /var/www/html/artisan schedule:run' | crontab -u www-data -
fi
